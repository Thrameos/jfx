...
public final class CSSPageRuleImplBinding {

    private static final Binder BINDER = new Binder(SymbolLookup.loaderLookup());

    private static final MethodHandle GET_SELECTOR_TEXT = bind(CSSPageRuleImplSignature.GET_SELECTOR_TEXT);
    private static final MethodHandle SET_SELECTOR_TEXT = bind(CSSPageRuleImplSignature.SET_SELECTOR_TEXT);
    private static final MethodHandle GET_STYLE = bind(CSSPageRuleImplSignature.GET_STYLE);

    private CSSPageRuleImplBinding() {
    }

    private static MethodHandle bind(CSSPageRuleImplSignature entry) {
    ...
    public static String getSelectorText(long peer) {
        try (Char16StringExchange exchange = new Char16StringExchange()) {
            ...
            throw Downcall.failed(CSSPageRuleImplSignature.GET_SELECTOR_TEXT.symbol(), t);
        }
    }
    ...
    public static void setSelectorText(long peer, String value) {
        try (Transfer utf8 = CString8.of(value, TransferPool.SHARED)) {
            ...
            throw Downcall.failed(CSSPageRuleImplSignature.SET_SELECTOR_TEXT.symbol(), t);
        }
    }
    ...
    public static long getStyle(long peer) {
        try {
            ...
            throw Downcall.failed(CSSPageRuleImplSignature.GET_STYLE.symbol(), t);
        }
    }
}
